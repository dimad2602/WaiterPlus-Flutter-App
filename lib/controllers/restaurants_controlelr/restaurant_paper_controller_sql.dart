...
class RestaurantPaperControllerSql extends GetxController {
  ...
  //TODO: Из-за того что метод вызывается на моменте загрузки приложения,
  // токена может ещё не быть. Поэтому рестораны не появляеются даже после авторизации. Не буду это исправлять, так как по логике
  // рестораны должны отображаться даже не авторизованному пользователю, авторизация появляеться если человек собираеться оформить заказ
